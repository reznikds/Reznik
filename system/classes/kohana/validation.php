...
class Kohana_Validation implements ArrayAccess {
		...
		foreach ($rules as $field => $set)
		{
			...
			foreach ($set as $array)
			{
				...
				if (is_array($rule))
				{
					...
					// Так как $rule - это массив, то он содержит инстанс класса($rule[0]) и название метода($rule[1]).
					// Это не подходит при проверке empty_rules, поэтому переменной $rule присваиваем название метода.
					$rule = $rule[1]; // mod by PS
				}
	...
	public function errors($file = NULL, $translate = TRUE)
	{
		...
		foreach ($this->_errors as $field => $set)
		{
			...
			if ((($message = Kohana::message($file, "{$field}.{$error}")) !== NULL) && is_string($message))
			{
				// Found a message for this field and error
			}
			elseif ((($message = Kohana::message($file, "{$field}.default")) !== NULL) && is_string($message))
			{
				// Found a default message for this field
			}
			elseif ((($message = Kohana::message($file, $error)) !== NULL) && is_string($message))
			{
				// Found a default message for this error
			}
			elseif ((($message = Kohana::message('validation', $error)) !== NULL) && is_string($message))
			{
				// Found a default message for this error
			}
			else
